...
export default function PaymentForm({ totalAmount, remainingAmount, onSubmit, isLoading = false }: PaymentFormProps) {
  ...
  const {
    ...
    reset: resetFull,
  } = useForm<FullPaymentFormData>({
    resolver: zodResolver(fullPaymentSchema),
    defaultValues: {
      paymentDate: new Date(),
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Full Payment Form */}
      {paymentType === 'full' && (
        <form onSubmit={handleSubmitFull(onSubmitFull)} className="space-y-4">
          <Input
            label="Data do Pagamento"
            type="date"
            icon={Calendar}
            {...registerFull('paymentDate', { valueAsDate: true })}
      ...
      {paymentType === 'partial' && (
        <form onSubmit={handleSubmitPartial(onSubmitPartial)} className="space-y-4">
          <Input
          ...
          <Input
            ...
            type="date"
            icon={Calendar}
            {...registerPartial('paymentDate', { valueAsDate: true })}
